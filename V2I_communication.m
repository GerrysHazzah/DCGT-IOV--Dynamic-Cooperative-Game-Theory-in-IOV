function V2I_communication(user_loc, bs_loc, power_user, power_B)
global radius l_i;
    l_i = 6.13e+03;
    communication_range = radius;
    distances = pdist2(bs_loc, user_loc);
    in_range_users = find(distances <= communication_range);

    % simulate  the process of sending a signal from the base station to the user
    for u = in_range_users
        received_power = power_B / (distances(u) ^ 2);

        % Determine whether the user can successfully receive the signal
        if received_power >= power_user
            disp(['User ', num2str(u), ' successfully receives signal from the BS.']);
        else
            disp(['User ', num2str(u), ' cannot receive signal from the BS due to low power.']);
        end
    end
end
